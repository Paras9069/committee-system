...
export default function UpdateDetails() {
  ...
  useEffect(()=>{
    let data={
        ...
        ApiServices.singleDetails(data).then((res)=>{
            ...
            setPerMemberAmount(res.data.data.perMemberAmount)
            setTotalAmount(res.data.data.totalAmount)
            setType(res.data.data.committeTypeId._id)
  ...
  const Submitted=(e)=>{
    ...
    ApiServices.updateDetails(data).then(
      (res)=>{
        console.log("response is:",res)
   
          toast.success(res.data.message)
          nav('/admin/manageDetails/')
      }
    ).catch(
      (err)=>{
        console.log(err);
